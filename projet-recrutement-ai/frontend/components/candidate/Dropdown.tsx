...
export default function Dropdown({
  ...
  return (
    ...
    >
      <Select.Trigger className="w-full bg-slate-50 border border-slate-200/80 rounded-xl px-3.5 py-2.5 flex items-center justify-between text-xs font-semibold text-slate-700 h-10 shadow-sm transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
        <span className="truncate text-left">{displayLabel}</span>
        <Icon icon="solar:alt-arrow-down-linear" className="w-3.5 h-3.5 text-slate-400 shrink-0 ml-2" />
      </Select.Trigger>
        ...
        >
          {options.map((opt) => (
            <ListBox.Item
              ...
              id={opt.id}
              textValue={opt.label}
              className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center outline-none select-none data-[selected=true]:bg-blue-50 data-[selected=true]:text-blue-600"
            >
              {opt.label}
