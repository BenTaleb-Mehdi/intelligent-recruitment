...
export default function SidebarCandidate({ isOpen, onClose }: SidebarCandidateProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-t border-slate-200/50 bg-slate-50/50 flex flex-col justify-end">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-blue-100 text-blue-600 flex items-center justify-center font-black text-xs select-none flex-shrink-0">
            MB
          </div>
          {isOpen && (
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-slate-800 truncate leading-none">Mehdi Ben Taleb</p>
              <p className="text-[10px] text-slate-400 truncate mt-1 leading-none">m.bentaleb@example.com</p>
            </div>
          )}
          <Link
            href="/"
            className={`p-2 text-slate-400 hover:text-slate-700 rounded-lg hover:bg-slate-250 transition-colors flex-shrink-0 ${
              !isOpen ? "mx-auto" : ""
            }`}
            title="Logout"
          >
            <Icon icon="solar:logout-bold" className="text-sm" />
          </Link>
        </div>
      </div>
    </aside>
