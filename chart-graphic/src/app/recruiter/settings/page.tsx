      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-xl sticky top-0 z-10 overflow-x-auto">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 text-xs font-semibold rounded-lg transition-all flex-1 justify-center ${
              activeTab === tab.id
      ...
      {activeTab === "profile" && (
        <div className="bg-white rounded-2xl border border-slate-200/70 shadow-sm p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              <Select className="w-full" placeholder="Sélectionner un secteur">
                <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Secteur d&apos;activité</Label>
                <Select.Trigger className="w-full bg-slate-50 border border-slate-200/80 rounded-xl px-4 py-2.5 flex items-center justify-between text-xs font-semibold text-slate-700 h-10 shadow-sm transition-colors cursor-pointer focus:outline-none focus:ring-1 focus:ring-blue-600">
                ...
                <Select.Popover className="border border-slate-200/80 shadow-lg rounded-xl bg-white p-1 z-30 mt-1 min-w-[220px]">
                  <ListBox className="text-xs font-medium text-slate-600">
                    <ListBox.Item id="tech" textValue="Technologies" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      Technologies
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    <ListBox.Item id="finance" textValue="Finance" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      Finance
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    <ListBox.Item id="sante" textValue="Santé" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      Santé
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    <ListBox.Item id="education" textValue="Éducation" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      Éducation
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    <ListBox.Item id="autre" textValue="Autre" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      Autre
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                  </ListBox>
                </Select.Popover>
                <Description>Sélectionnez le secteur d&apos;activité de votre entreprise</Description>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Select className="w-full" placeholder="Sélectionner la taille">
                <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Taille de l&apos;équipe</Label>
                <Select.Trigger className="w-full bg-slate-50 border border-slate-200/80 rounded-xl px-4 py-2.5 flex items-center justify-between text-xs font-semibold text-slate-700 h-10 shadow-sm transition-colors cursor-pointer focus:outline-none focus:ring-1 focus:ring-blue-600">
                  <Select.Value className="text-left" />
                  <Select.Indicator>
                    <Icon icon="solar:alt-arrow-down-linear" className="w-3 h-3 text-slate-400" />
                ...
                <Select.Popover className="border border-slate-200/80 shadow-lg rounded-xl bg-white p-1 z-30 mt-1 min-w-[220px]">
                  <ListBox className="text-xs font-medium text-slate-600">
                    <ListBox.Item id="1-10" textValue="1-10" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      1-10
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    <ListBox.Item id="10-50" textValue="10-50" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      10-50
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    <ListBox.Item id="50-200" textValue="50-200" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      50-200
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                    <ListBox.Item id="200+" textValue="200+" className="px-3 py-2 rounded-lg hover:bg-slate-100/80 cursor-pointer flex justify-between items-center">
                      200+
                      <ListBox.ItemIndicator />
                    </ListBox.Item>
                  </ListBox>
                </Select.Popover>
                <Description>Indiquez le nombre d&apos;employés dans votre entreprise</Description>
              </Select>
            </div>
